...
from dataload.data_preprocess import prepare_preprocessed_data as prepare_preprocessed_data_tsv
from dataload.data_preprocess_parquet import prepare_preprocessed_data as prepare_preprocessed_data_parquet
...
# Changed default config_name to ebnerd
@hydra.main(version_base="1.2", config_path=os.path.join(get_root(), "configs"), config_name="ebnerd")
def main(cfg: DictConfig):
    # Added to make sure we know what parameters are being used
    print(cfg)
    seed_everything(cfg.seed)
    cfg.gpu_num = torch.cuda.device_count()
    print("data format:", cfg.dataset.data_format)
    if cfg.dataset.data_format == "TSV":
        prepare_preprocessed_data_tsv(cfg)
    # Added to preprocess data if it's a parquet file
    elif cfg.dataset.data_format == "parquet":
        prepare_preprocessed_data_parquet(cfg)
    else:
        raise Exception("Invalid value for data_format:", cfg.dataset.data_format)
